...
class GetOrdersWithNotUpdatedStatus extends \MageSuite\NotificationDashboard\Model\Command\Notification\CollectAndSend
{
    ...
    public function execute()
    {
        ...
        $ordersInfo = $this->orderResource->getOrdersWithSpecificStatus($maxDate, $minDate, $configuration['status']);

        if (empty($ordersInfo)) {
            return;
        }

        $statusLabel = $this->orderConfig->getStatusLabel($configuration['status']);

        foreach ($ordersInfo as $orderInfo) {
            $this->addNotification->execute(
                __(
                    "Order #%1 has still %2 status and it was created at %3.",
                ...
                __('Order with not updated status')
            );
        }
    }
}
